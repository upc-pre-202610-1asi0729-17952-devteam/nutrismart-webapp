 ...
 */
export type ProfilePanel = 'personal' | 'physical' | 'dietary' | 'subscription' | 'security';
 ...
 * 4. Subscription — plan management via {@link SubscriptionPlans}
 * 5. Security and privacy — change password, session info, danger zone
 */
@Component({
  selector: 'app-profile',
  standalone: true,
  imports: [ReactiveFormsModule, TranslatePipe, BillingPanel],
...
export class Profile implements OnInit {
  ...
  activeNavItem = computed(() => this.navItems.find(i => i.id === this.activePanel()));
